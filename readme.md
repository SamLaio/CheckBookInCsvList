...
這是一個強大的油猴腳本 (Userscript)，讓你能在任何網頁上透過「選取文字」即時比對你在 Google 試算表（Google Sheets）中紀錄的資料。支援模糊搜尋、多結果顯示，並具備可拖拽的現代化 UI。

---

## ✨ 功能特色

- **☁️ 實時雲端同步**：直接讀取 Google 試算表發佈的 CSV 連結，資料更新免改代碼。
...
2. 將本專案的 `script.js` 內容完整貼上。
3. **修改參數：** 在腳本最上方的 `CONFIG_CSV_URL` 處填入你剛才複製的 CSV 網址。
4. 儲存 (Ctrl+S)。
